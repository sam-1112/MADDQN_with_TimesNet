...
class MADDQN:
    ...
    def initialize(self):
        ...
        self.data_processor = PreprocessData(data=self.unprocessed_data, window_size=self.configs['env']['window_size'], dates=dates, multi_data=self.args.mode)
        ...
        self.plot_utils = PlotUtils(configs=self.configs)
    ...
    def training_process(self):
        ...
        # 最終獎勵分佈分析
        self.analyze_reward_distribution()
        self.plot_utils.plot_reward_distribution(self.reward_stats)
        self.plot_utils.plot_episode_returns(self.episode_returns)
        self.plot_utils.plot_loss_trends(self.reward_stats)
        self.plot_utils.plot_stock_history(self.unprocessed_data)
    ...
    def testing_process(self):
        ...
        model_paths_pattern = glob.glob(os.path.join(self.configs['training']['model_save_dir'], f"*agent_ep{self.configs['training']['episodes']}_*.pth"))
        ...
        def extract_timestamp(filename):
            ...
            # 假設格式為: agent_type_ep1_YYYYMMDD_HHMMSS.pth
            if len(parts) >= 3:
                date_part = parts[-2]  # YYYYMMDD
                time_part = parts[-1].replace('.pth', '')  # HHMMSS
                return f"{date_part}_{time_part}"
            return "00000000_000000"

        risk_agent_model_path = sorted(risk_agent_model_path, key=extract_timestamp)
        return_agent_model_path = sorted(return_agent_model_path, key=extract_timestamp)
        final_agent_model_path = sorted(final_agent_model_path, key=extract_timestamp)
        
        model_path = [
            risk_agent_model_path[-1] if risk_agent_model_path else None,
            return_agent_model_path[-1] if return_agent_model_path else None,
            final_agent_model_path[-1] if final_agent_model_path else None
        ]
        try:
        ...
        # 繪製測試結果
        self.plot_utils.plot_test_results(self.test_cumulative_returns, self.market_return, self.test_portfolio_values_final, self.test_episode_rewards)
